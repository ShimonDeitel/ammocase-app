...
struct ContentView: View {
    ...
    @State private var editingItem: LotItem?
    ...
    @ViewBuilder
    private func row(for entry: LotItem) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(entry.name)
                .font(Theme.headlineFont)
                .foregroundColor(Theme.textPrimary)
            Text("Caliber: \(entry.caliber)  \u{2022}  Quantity: \(entry.quantity)")
                .font(Theme.captionFont)
                .foregroundColor(Theme.textMuted)
        }
        .padding(.vertical, 4)
...
struct ItemEditView: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section {
                    ...
                    TextField("Caliber (9mm)", text: $field1)
                        .focused($focusedField, equals: .field1)
                        .accessibilityIdentifier("field1Field")
                    TextField("Quantity (250)", text: $field2)
        ...
        .onAppear {
            if case .edit(let entry) = mode {
                name = entry.name
                field1 = entry.caliber
                field2 = entry.quantity
                notes = entry.notes
            }
        }
    }
    ...
    private func save() {
        switch mode {
        case .add:
            store.add(name: name, caliber: field1, quantity: field2, notes: notes)
        case .edit(var entry):
            entry.name = name
            entry.caliber = field1
            entry.quantity = field2
            entry.notes = notes
            store.update(entry)
        }
        dismiss()
